    ...
    if dispatch_result.vehicle_routes:
        ...
        if util_rows:
            ...
            fig = px.bar(
                ...
                title="车辆单趟利用率",
                range_y=[0, 1],
                text=df_util["利用率"].apply(lambda x: f"{x:.0%}"),
            )
            fig.update_layout(height=250, margin=dict(l=20, r=20, t=30, b=20))
            fig.update_traces(textposition="outside")
            st.plotly_chart(fig, use_container_width=True)
            st.dataframe(
                df_util.style.format({"利用率": "{:.0%}"}),
...
def render_dispatch_params() -> dict:
    ...
    n_buses = st.sidebar.slider("公交车数量", 3, 40, 15, step=1)
    bus_capacity = st.sidebar.slider("单车容量 (人)", 30, 150, 80, step=10)
